...
db = SQLAlchemy(app)
## This is like defining a DB Table with columns like id which is primary key, title and complete.
class Todo(db.Model):
   id= db.Column(db.Integer, primary_key=True)
   title=db.Column(db.String(100))
   complete= db.Column(db.Boolean)
## This is the index route where we are quering all the rows which are in our DB and passing it to todos
@app.route('/')
def index():
...
## This is used to add a todo into our DB
